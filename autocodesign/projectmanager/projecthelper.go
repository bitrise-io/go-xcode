...
func mainTargetOfScheme(proj xcodeproj.XcodeProj, scheme xcscheme.Scheme) (xcodeproj.Target, error) {
	fmt.Printf("Searching %v for scheme main target: %s", len(scheme.BuildAction.BuildActionEntries), scheme.Name)
	...
	fmt.Printf("Searching %v targets for: %s", len(proj.Proj.Targets), blueIdent)
...
func findBuiltProject(pth, schemeName string) (xcodeproj.XcodeProj, xcscheme.Scheme, error) {
	log.TInfof("Location built project for xcode project: %s, scheme: %s", pth, schemeName)
