...
func TestIsXcodeManaged(t *testing.T) {
	...
	managed, _ := archive.IsSigningManagedAutomatically()
	require.Equal(t, false, managed)
}
...
func Test_applicationFromPlist(t *testing.T) {
	infoPlist, err := plistutil.NewMapDataFromPlistFile(filepath.Join(sampleRepoPath(t), "archives/ios.xcarchive/Info.plist"))
...
func Test_GetAppLayout(t *testing.T) {
	tests := []struct {
		name    string
		archive IosArchive
