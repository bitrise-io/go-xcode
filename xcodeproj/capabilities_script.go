...
const capabilitiesScriptContent = `require 'xcodeproj'
require 'json'

def read_target_name(project, target_uuid)
  project.targets.each do |target|
    return target.name if target.uuid == target_uuid
  end
  nil
end
...
def read_target_capabilities(project_path)
  ...
  target_attributes.each do |current_uuid, current_attributes|
    ...
    target_name = read_target_name(project, current_uuid)

    target_capabilities[target_name] = capability_names
  end
