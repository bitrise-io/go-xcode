...
func TestAppStoreExportOptionsGeneratorForAutomaticSigning(t *testing.T) {
	tests := []struct {
		name              string
		teamID            string
		options           AppStoreOptionalOptions
		wantExportOptions string
	}{
		{
			name:   "default export options",
			teamID: "MY_TEAM_ID",
			wantExportOptions: `<?xml version="1.0" encoding="UTF-8"?>
...
<dict>
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			generator := NewGeneratorForAppStoreExports(tt.teamID, tt.options)
			options := generator.GenerateForAutomaticSigning()
			wantOptions, err := unmarshalExportOptions([]byte(tt.wantExportOptions))
			require.NoError(t, err)
			require.EqualValues(t, wantOptions, options)
		})

	}
}

func TestAppStoreExportOptionsGeneratorForManualSigning(t *testing.T) {
	tests := []struct {
		name                 string
		teamID               string
		signingCertificate   string
		provisioningProfiles map[string]string
		options              NonAppStoreOptionalOptions
		wantExportOptions    string
	}{
		{
			name:               "default export options",
			...
			provisioningProfiles: map[string]string{
				"com.example.app":         "Apple Development com.example.app",
				"com.example.watchkitapp": "Apple Development com.example.watchkitapp",
			},
			wantExportOptions: `<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
	...
	<dict>
		...
		<string>Apple Development com.example.app</string>
		<key>com.example.watchkitapp</key>
		<string>Apple Development com.example.watchkitapp</string>
	</dict>
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			generator := NewGeneratorForNonAppStoreExports(MethodDebugging, tt.teamID, tt.options)
			options := generator.GenerateForManualSigning(tt.signingCertificate, tt.provisioningProfiles)
			optionsBytes, err := marshalExportOptions(options)
			require.NoError(t, err)

			wantOptions, err := unmarshalExportOptions([]byte(tt.wantExportOptions))
			require.NoError(t, err)

			require.EqualValues(t, wantOptions, options, string(optionsBytes))
		})
